...
import { ExpenseCategory, PersonalExpense, TabExpenseCategory } from "../../model/expense.model";
...
export class AddEditPersonalExpenseComponent {
    ...
    activeTab(event: ExpenseCategory) {
        this.tabs.forEach((val: TabExpenseCategory) => {
            val.items.forEach((k: ExpenseCategory) => {
                k.active = event.name===k.name;
