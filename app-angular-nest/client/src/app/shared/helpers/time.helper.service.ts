...
export class TimeHelpersService {
    ...
    formatDateTime(
        ...
        timeFormat: ETimeFormat = this.settings.timeFormat || ETimeFormat.DEFAULT
    ) {
        const value = milliseconds || dateStr;
        if (this.isValidInput(value, dateFormat, timeFormat)) {
            return "";
        }
    ...
    formatTime(
        ...
        const value = milliseconds || dateStr;
        if (this.isValidInput(value, null, timeFormat)) {
            return "";
        }
     ...
     */
    getTimeRange(duration: EDuration, numberOfYearsAgo: number = 0): {
        startDate: Date;
        endDate: Date;
    } {
        const today = dayjs();
     ...
     */
    getTodayTimeRange(): { startTime: Date; endTime: Date } {
        const today = dayjs();
     ...
     */
    getDaysAgo(day: string, dayAgo: number): Date[] {
     ...
     * @returns An array of dates.
     */
    getDaysAfter(day: any, dayAfter: number): Date[] {
        const currentDate = dayjs(day);
     ...
     */
    sortObjectByKeyIsDate(obj: any): any {
