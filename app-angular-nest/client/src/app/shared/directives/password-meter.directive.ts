...
export class PasswordMeterDirective implements OnChanges {
    @Input() appPasswordMeter: string = "";
    private strengthScore: string = "";
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (changes["appPasswordMeter"]) {
            if (changes["appPasswordMeter"].currentValue === "") {
                ...
                const newScore = zxcvbn(changes["appPasswordMeter"].currentValue).score;
    ...
    private renderStrengthMeter() {
        ...
        const targetElement = this.el.nativeElement.querySelector(".mat-mdc-form-field-subscript-wrapper");
        divElement.className = this.passwordStrengthClass;
        divElement.setAttribute(this.attributeStrengthHtml, this.strengthScore);

        const meterBarElements = Array.from({ length: this.numberMeterBar }, (_) => {
        ...
        this.renderer.insertBefore(this.el.nativeElement, divElement, targetElement);
    }

    private updateStrengthAttribute(strength: string) {
        const strengthDiv = this.el.nativeElement.querySelector("." + this.passwordStrengthClass);
        if (strengthDiv) {
            this.renderer.setAttribute(strengthDiv, this.attributeStrengthHtml, strength);
        }
    }
